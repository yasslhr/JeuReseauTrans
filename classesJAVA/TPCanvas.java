...
public class TPCanvas extends Canvas {
	...
	int size =700;     //taille de la fenetre (px)
	int nbPosition = 10;
	byte [] etat;       
	...
	{
		System.out.println("Paint: update du canvas.");
		paintCarte(win);
		drawEtat(win);
	...
	{
		for (int i=0;i<10*10;i++)       //on parcourt chaque case du tableau
		{
			if (etat[2*i]!=0)       //Si le joueur est sur cette case, lafficher
			{       
